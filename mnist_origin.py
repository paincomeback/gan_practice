...
parser.add_argument('--name', type=str, default='./mnist_ori/my_origin')
...
loss_gen = T.mean(T.nnet.softplus(-output_before_softmax_gen))
...
for epoch in range(300):
    ...
    for i in range(args.batch_size):
        img_list.append(img_bhwc[i,:,:])

    mu, std = inception_score.get_inception_score(img_list)

    img_tile = plotting.img_tile(img_bhwc, aspect_ratio=1.0, border_color=1.0, stretch=True)
    img = plotting.plot_img(img_tile, title='MNIST samples')

    # report
    print("Iteration %d, time = %ds, loss_unl = %.4f, loss_gen = %.4f, mu = %.4f, std = %.4f" %(epoch, time.time()-begin, loss_unl, loss_gen, mu, std))
    f.write("Iteration %d, time = %ds, loss_unl = %.4f, loss_gen = %.4f, mu = %.4f, std = %.4f\n" %(epoch, time.time()-begin, loss_unl, loss_gen, mu, std))
    sys.stdout.flush()
